/*
Best fit Grapher given user points and degree interchangable: Is able to produce the proper matrix of coefficients needed to plot a best fit of an equation of a given degree (currently set to 2 as default)

Authors: Abhishek Das and Shrikrishna Bhat
...
int main()
{
  ...
  for (int i = 0; i <= degree; i++){
    for (int j = 0; j <= degree +1; j++){
      ...
      // For last row to compute sum of x^i * y
      if (j == degree + 1){
  ...
  int success = gaussian_elimination(matrix, degree+1, coeff);
  if (success == 0){
    printf("Gaussian algorithm failed.\n");
    return 0;
  }

  // graph the Function using Y = ax^0 + bx^1 + cx^2
  for (X = 0.0; X < 600.0; X+=0.1){
    Y = 0.0;
  ...
  G_save_image_to_file("poyBestFit.bmp") ;
}